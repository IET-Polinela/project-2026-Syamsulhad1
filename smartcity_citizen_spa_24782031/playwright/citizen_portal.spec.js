...
async function loginAdmin(page, username, password) {
    ...
    // Klik submit dan cukup tunggu navigasi commit.
    // Hindari load/networkidle karena mode headed bisa tertahan asset eksternal.
    await Promise.all([
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-01: Chart.js di Dashboard Admin ter-render dengan benar', async ({ page }) => {
        ...
        //
        const chartCanvasesReady = await page.evaluate(() => {
            ...
            return Boolean(
                statusCanvas
                && categoryCanvas
                && statusCanvas.getBoundingClientRect().width > 0
                && categoryCanvas.getBoundingClientRect().width > 0
            );
        });

        expect(chartCanvasesReady).toBe(true);

        // -------------------------------------------------------------------
        // LANGKAH 6: Verifikasi tabel data juga ada
        // -------------------------------------------------------------------
        // Dashboard juga menampilkan 2 tabel: reportedTable dan resolvedTable
        await expect(page.locator('#latestReportedRows')).toBeVisible();
        await expect(page.locator('#latestResolvedRows')).toBeVisible();
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        await page.goto(`${SPA_URL}#dashboard`);
        await page.waitForSelector('#newReportButton', { state: 'visible', timeout: 10000 });

        // -------------------------------------------------------------------
        // LANGKAH 4: Klik tab "Feed Kota (Publik)"
        // -------------------------------------------------------------------
        // Tab ini ada di router.js (template #dashboard), id='tabFeedKota'
        const tabFeedKota = page.locator('[data-tab="feed"]').first();
        ...
        const listContainer = page.locator('#reportList');
        await expect(listContainer).toBeVisible();

        const reportCards = listContainer.locator('.feed-card, .report-card');
        ...
        const paginationContainer = page.locator('#paginationBar');
    ...
    test('UI-04: Klik tombol Buat Laporan → modal #reportModal muncul', async ({ page }) => {
        ...
        const btnBukaModal = page.locator('#newReportButton');
        ...
        await expect(modalTitle).toContainText('Laporan Baru');
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        await page.locator('#reportCategory').selectOption('INFRASTRUCTURE');
    ...
    test('UI-06: Responsive navbar pada viewport mobile (400x800)', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        const dashboardShell = page.locator('.dashboard-app');
        const sidebar = page.locator('.app-sidebar');
        await expect(dashboardShell).toBeVisible({ timeout: 5000 });
        await expect(sidebar).toBeVisible({ timeout: 5000 });
        const navbar = dashboardShell;
        ...
        if (togglerCount > 0) {
            ...
            const navMenus = page.locator('.app-sidebar-nav');
